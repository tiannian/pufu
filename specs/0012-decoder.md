...
- **`get_var(&self, idx: u32) -> Result<&[u8], CodecError>`**  
  Retrieves the `idx`-th variable-length entry slice. Interprets the `idx`-th VarEntry `u32` (per `specs/0010-binary-serde.md`) as an absolute payload offset into the Data region and uses the next VarEntry offset (or `total_len` for the final entry) to compute the end. Performs bounds checks: the start must be at least `data_offset`, offsets must not decrease, and the final range must stay inside `[data_offset, total_len]` and `buf.len()`. Returns a borrowed slice tied to `'a`; `FieldDecode` implementations consume it to rebuild var1/var2 payloads without copying when possible.
